...
### Phase 2: AI 장표 생성 + 편집기
- [x] Gemini API 연동 (코드 생성 완료, 실행 확인 필요)
- [x] 장표 편집 페이지 (코드 생성 완료, 실행 확인 필요)
- [x] 장표 라이브러리 (코드 생성 완료, 실행 확인 필요)
- [x] **HWP→HTML 변환 (pyhwp)** — 구현 + 테스트 완료
- [x] HWP 변환 테스트 페이지 (/hwp)
- [ ] Gemini 실행 확인 (API 키 설정 후)
- [ ] 편집기에 HWP 변환 결과 로드 연동
